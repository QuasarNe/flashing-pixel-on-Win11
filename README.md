...
1. 手机冷启动进入fastboot模式（关机状态下长按电源键和音量下键），连接电脑，或者手机连接电脑后，启动cmd/Powershell,输入`fastboot devices`，若出现设备信息则连接成功。输入`adb reboot bootloader`进入fastboot模式
...
3. 在设备端此时会弹出 Bootloader 解锁的确认界面，使用音量键移动光标选择确认，按下电源键开始解锁
4. 解锁后设备会被清除数据并重新启动，待设备开机后检查「USB 调试」选项；重新进入 Bootloader 界面等候
